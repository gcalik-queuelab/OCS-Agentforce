...
import createSummary from '@salesforce/apex/AF_NPSSummaryCreator.createSummary';

export default class NpsSummaryRecreateConfirm extends LightningElement {
    @api recordId;
    ...
    /**
     * Creates the summary record (automatically deletes old one if exists)
     */
    createSummaryRecord() {
        if (!this.recordId) {
        ...
        createSummary({ npsId: this.recordId })
            .then(result => {
                if (result) {
